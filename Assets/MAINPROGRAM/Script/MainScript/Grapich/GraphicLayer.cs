...
public class GraphicLayer
{
    ...
    public Coroutine SetTexture(string filePath, float transitionSpeed = 1f, Texture blendingTexture = null, bool immediate = false)
    {
        ...
        if(tex == null)
        {
            Debug.LogError($"Could not load Texture from path '{filePath}'. please ensure it exist within Resource!");
            return null;
        }
    ...
    public Coroutine SetVideo(string filePath, float transitionSpeed = 1f, bool useAudio = true, Texture blendingTexture = null, bool immediate = false)
    {
        ...
        if (clip == null)
        {
            Debug.LogError($"Could not load Video from path '{filePath}'. please ensure it exist within Resource!");
            return null;
        }
    ...
    private Coroutine CreateGraphic<T>(T graphicData, float transitioSpeed, string filePath, bool useAudioVideo = true, Texture blendingTexture = null, bool immediate = false)
    {
        ...
        if (graphicData is Texture)
            ...
            newGraphic = new GraphicObject(this, filePath, graphicData as VideoClip, useAudioVideo, immediate);
        ...
        if(!immediate)
            return currentGraphic.FadeIn(transitioSpeed, blendingTexture);

        //Otherwise thiis an immediate
        DestroyOldGraphic();
        return null;
    }
